...
<body>
  ...
  <tr>
    <td> <form id="form1" name="form1" method="post" action="/shi.php/Home/Index/pin_list">
	 
   <input name="user" type="text" class="dfinput" id="user" />
	<input name="" type="submit" class="btn" value="确认搜索"/>
      </form></td>
    <td align="right"><a href="/shi.php/Home/Index/pin_list/cz/0/">未使用</a> <a href="/shi.php/Home/Index/pin_list/cz/1/">已使用</a> </td>
  </tr>
</table>
		...
		<?php if(is_array($list)): foreach($list as $key=>$v): ?><tr>
		 ...
		 <td><?php echo ($v["user"]); ?></td>
		  <td><?php echo ($v["pin"]); ?></td>
		   <td><?php if($v["zt"] == 0): ?>未使用<?php else: ?>已使用<?php endif; ?></td>
		    <td><?php echo ($v["sc_date"]); ?></td>
        
        <td><?php echo ($v["sy_user"]); ?></td>
        <td><?php echo ($v["sy_date"]); ?></td>
        <td><a onClick="javascript:if(!confirm('确定删除此PIN码？'))  return  false; " href="/shi.php/Home/Index/pin_del/id/<?php echo ($v["id"]); ?>" >删除</a></td>
