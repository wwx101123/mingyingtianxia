...
//  }    
   ...
   /* array (
	...
	public function zdpp_cl(){
		...
		$jsbz_list=M('jsbz')->where(array('zt'=>0))->order('date asc')->select();
		//要匹配的的额度
		$jsbz_total=M('jsbz')->where(array('zt'=>0))->field('sum(jb) as jb')->find();
		//查询今天已经匹配的订单
    ...
    public function jjset()
    {
        if (IS_POST) {
            ...
            //打款后分红天数----------------------------------->by QQ450269178
            $this->jjfhdays = C("jjfhdays");
            //排队分红天数----------------------------------->by QQ450269178
