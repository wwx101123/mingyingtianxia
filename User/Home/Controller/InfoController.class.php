...
//jerry
    ...
    public function tgbz_tx_cl()
    {
        ...
        if (I('get.id') <> '') {
            ...
            if ($count1 == C('daybenxi')) {
					//die("<script>alert('提现失败，每天只允许提现".C('daybenxi')."次！');history.back(-1);</script>");
					$this->error("提现失败，每天只允许提现".C('daybenxi')."次！");
					exit;
            } else {
                ...
                if ($count2 >= C('monthbenxi')) {
                    //die("<script>alert('提现失败，每月只允许提现".C('monthbenxi')."次！');history.back(-1);</script>");

					$this->error("提现失败，每月只允许提现".C('monthbenxi')."次！");
					exit;
                } else {
                    //
                    $varid = I('get.id');
                    $proall = M('user_jj')->where(array('id' => $varid))->find();
                    if ( $proall['status'] == '2' || $proall['status'] == 2 ) {
						$this->error("转出失败,交易未完成！");
						exit;
                        //die("<script>alert('转出失败,交易未完成！');history.back(-1);</script>");
                    } else {
                        ...
                        if(C('jjdjdays')>0){
                            ...
                            if($diff_day < C('jjdjdays')){
								...
								$this->error("打款完后要等".C('jjdjdays')."天才可以提现哦！，不过你现在只需等".$wait_day."天就可以提现了！");
								exit;
                        ...
                        $lx_he = user_jj_zong_lx($varid) + $proall['jb'];             //-------------------------------->修复利息计算错误
                        $user_zq = M('user')->where(array('UE_ID' => $_SESSION['uid']))->find();
                        $user_xz = M('user')->where(array('UE_ID' => $_SESSION['uid']))->find();
						if( $proall['zt'] == '1' || $proall['zt'] == 1 ){
							$this->error("您已经提款了！");
							exit;
						}else{

							M()->startTrans();
							$u = M('user')->where(array('UE_ID' => $_SESSION['uid']))->setInc('UE_money', $lx_he);
							$j = M('user_jj')->where(array('id' => $varid))->save(array('zt' => '1'));
...
///////////////////////////
	 ...
	 public function pai()
    {
        ...
        $list1 = $User->where($map1)->order('zt ASC')->limit($p1->firstRow, $p1->listRows)->select();
        $this->assign('list1', $list1); // 賦值數據集
        $this->assign('page1', $p1->show()); // 賦值分頁輸出
        /////////////////----------------

        $this->pin_zs = M('pai')->where(array('user' => $_SESSION['uname'], 'zt' => 0))->count() + 0;
    ...
    public function paitopin(){
        //导航激活
        $this->pin_zs = M('pai')->where(array('user' => $_SESSION['uname'], 'zt' => 0))->count();
        $this->manage = true;
        $this->display();
    }
   ...
   public function rwhistory(){
    ...
    $list1 = $User->where($map1)->order('r_id Asc')->limit($p1->firstRow, $p1->listRows)->select();
	$Num=0;
    $list2 = $User->where($map1)->order('r_id Desc')->limit(0, 1)->select();
    $Rid  = $list2[0]['r_id'];
   ...
   public function nwhistory(){
    ...
    public function txtdj(){
        $id = I('get.id',0,'intval');
        $ob = M('user_jl')->where("id={$id} AND zt=0")->find();
        ...
        M('user')->where(array('UE_account'=>$ob['user']))->setInc($qb,$ob['jb']);
        M('user_jl')->where("id={$id} AND zt=0")->setField('zt',1);
        $this->success("转出成功！");
    }
