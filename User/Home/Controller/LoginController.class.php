...
class LoginController extends Controller {
    ...
    public function logincl() {
    	...
    	if (IS_POST) {
			...
			if(!$this->check_verify($code)){
                $this->ajaxReturn( array('nr'=>'验证码错误！','sf'=>1) );
            }
			if(false){
				$_SESSION['log_codes'] = 1;
			...
			if(!$user || $user['ue_password']!=md5($pwd) || $_SESSION['log_codes'] == 1){ 

				$_SESSION['log_codes'] = 1;
				//$this->ajaxReturn( array('nr'=>'賬號或密碼錯誤!','sf'=>1) );
                $this->ajaxReturn( array('nr'=>'输入密码错误賬號或密碼錯誤, 需要等待<span id="w_second">60</span>秒后,才能再次输入!','sf'=>2) );

			}elseif($user['ue_status']=='1'){
				$this->ajaxReturn( array('nr'=>'賬號被禁用!','sf'=>1) );
			}elseif($user['ue_check']==0){
                $this->ajaxReturn( array('nr'=>'當前賬戶未激活，暫不能登陸!','sf'=>1) );
            }else{
				...
				$data = array(
					'UE_nowTime' => date ( 'Y-m-d H:i:s', time ()),
					'UE_nowIP'   => I('post.ip'),
					'UE_lastTime'=> $user['UE_nowTime'],
					'UE_lastIP'  => $user['UE_nowIP'], 
				);
				M('user')->where(array('UE_account'=>$username))->save($data);
	...
	public function logcodes() {
		$_SESSION['log_codes'] = 0;
	}
    ...
    public function cspaycl ($data)
    {
        ...
        if ( $rs ){
        	...
        	if ( $cszt ){
                ...
                $data = array();
                $data['zt'] = -1;
                $tgbz->where(array('user'=>$uname))->save($data);
        		$jsbz = M('jsbz');
				$data = array();
                $data['zt'] = -1;
                $tgbz->where(array('user'=>$uname))->save($data);
				...
				$ppdd_rs=$ppdd->where($where)->select();
				foreach($ppdd_rs as $rs){
					M('tgbz')->where(array('id' => $rs['p_id']))->save(array('zt' => -1, 'qr_zt' => 0));
        			M('jsbz')->where(array('id' => $rs['g_id']))->save(array('zt' => 0, 'qr_zt' => 0));
        			M('ppdd')->where(array('id' => $rs['id']))->delete();
				}
