...
// 			$this->display ( 'lxwmx' );
	...
	public function active(){
		...
		if(IS_POST){
			if(!M('pai')->where("pin='".$_POST['sh']."'")->find()){
				$this->error("激活码错误！");
			}
			if(M('user')->where("UE_account='".$user."'")->setField('UE_check',1)){
				//更新激活码状态
				$data = array(
					'zt'=>1,
					'sy_user'=>$user,
				...
				M('pai')->where("pin='".$_POST['sh']."'")->data($data)->save();
				$this->success("用户激活成功！");
				exit;
			}else{
				$this->error("用户激活失败！");
		...
		$this->code = M('pai')->where(array('user' => $_SESSION['uname'], 'zt' => 0))->getField('pin');
		$this->display();
	}	
